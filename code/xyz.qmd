...
Une fonction de base très utile est `table()`. Celle-ci calcule le nombre de valeurs uniques d'une variable. On peut ensuite faire des statistiques sur cette distribution.

```{r}

# La fonction `table( )` permet d'observer le nombre de modalités uniques d'une variable. 
...
# 1. Syntaxe étendue -- plus explicite: le résultat de chaque opération est emmagasiné dans une variable -- plus d'objets sont cependant générés et stockés en mémoire
